...
class FramePanel(tk.Tk):
    ...
    # Рразмещение панели вкладок и панели рисования в главном окне
...
class FugureFillControlPanel(tk.Frame):
    ...
    # Закраска фигуры модицицированным рекурсивным алгоритмом
    def fill(self):
        x0 = int(self.start_x.get())
        y0 = int(self.start_y.get())
...
class Algorithms:
    ...
    def parameter_lissajous_drawing(board, center, r_x, r_y):
        ...
        for t in range(0, 360):
            x = int(r_x * math.sin(t * (math.pi / 180)))
            y = int(r_y * math.sin(t * (math.pi / 180)))
